...
import { NotFoundScreen as NotFound } from "./NotFoundScreen";
import { UI } from "@/lib/i18n/ui";

describe("tela de endereço inexistente", () => {
  it("diz que a página não existe, sem o texto técnico do framework", () => {
    render(<NotFound />);

    expect(screen.getByRole("heading", { name: UI.en.notFound.title })).toBeDefined();
  ...
  it("oferece o caminho de volta", () => {
    ...
    expect(screen.getByRole("link", { name: UI.en.notFound.back }).getAttribute("href")).toBe("/");
  });
  ...
  it("não fala de whiteboard: aqui o endereço nunca existiu", () => {
    ...
    expect(document.body.textContent ?? "").not.toContain(UI.en.boardNotFound.title);
  });
});
